...
import { Users, Settings, FileText, Activity, LogOut, Building, User, Loader2 } from 'lucide-react';
...
const Dashboard: React.FC = () => {
  ...
  const handleSelection = async (selection: 'company' | 'individual') => {
    setShowModal(false);
    try {
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL}/auth/update-dashboard-type`, {
        method: 'POST',
        headers: {
    ...
    <div className="min-h-screen bg-black text-white">
      <nav className="flex items-center justify-between p-6 bg-gray-900">
        <Link href="/" className="flex items-center space-x-2">
          <span className="text-2xl font-bold text-white">TESTFIRE</span>
        </Link>
        <ul className="flex space-x-8 items-center">
          <li>
            <Link href="/blog" className="text-gray-400 hover:text-blue-500 transition text-lg">
              Blog
            </Link>
          </li>
          <li>
            <Link href="/pricing" className="text-gray-400 hover:text-blue-500 transition text-lg">
              Pricing
            </Link>
          </li>
          <li>
            <Link href="/docs" className="text-gray-400 hover:text-blue-500 transition text-lg">
              Docs
            </Link>
          </li>
          <li>
      ...
      {showModal && (
        <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 animate-[fadeIn_0.5s_ease-in]">
          <div className="bg-gray-900 p-8 rounded-xl shadow-lg text-center space-y-6 max-w-md w-full animate-[fadeIn_0.5s_ease-in] border border-gray-700">
            <h2 className="text-2xl font-bold text-white">Choose Your Dashboard</h2>
        ...
        <div className="max-w-7xl mx-auto px-6 py-12">
          <main>
            <h1 className="text-4xl md:text-5xl font-bold text-center mb-8 text-white animate-[fadeIn_0.5s_ease-in]">
              WELCOME TO YOUR TESTFIRE DASHBOARD
            </h1>
            <p className="text-gray-400 text-center max-w-2xl mx-auto mb-12 text-lg animate-[fadeIn_0.5s_ease-in]" style={{ animationDelay: '0.2s' }}>
              Manage your apps, track testing progress, and view detailed reports with TestFire’s AI-driven platform.
            </p>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
              ...
              >
                ...
                <p className="text-gray-400">View system logs and activities.</p>
              </Link>
            </div>
          </main>
        </div>
...
// // Comment out AuthContext and useUserRequest for testing
// import { AuthContext } from '@/context/AuthContext';
// import { useUserRequest } from '@/hooks/auth';
// interface User {
//   id: string;
...
//   // Comment out authentication logic for testing
...
//     const dashboardType = userData?.data?.dashboardType;
//     if (dashboardType) {
//       setShowModal(false);
...
//     try {
//       await fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL}/auth/update-dashboard-type`, {
//         method: 'POST',
//         headers: {
//           'Content-Type': 'application/json',
//           Authorization: `Bearer ${localStorage.getItem('token')}`,
...
//   // Remove loading and error states for testing
//   if (isLoading || isAuthenticating) {
//     return (
//       <div className="min-h-screen bg-black text-white flex items-center justify-center">
//         <div className="flex flex-col items-center animate-[fadeIn_0.5s_ease-in]">
